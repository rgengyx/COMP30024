...
def find_3x3_adjacent_squares(n, square, layout, our_colour):
    ...
    adjacent_coordinates = [(i, j) for i in range(x - n, x + n + 1) for j in range(y - n, y + n + 1) if
                            0 <= i <= 7 and 0 <= j <= 7 and (i, j) != (x, y)]
    our_squares = layout[our_colour]
    return [token for token in our_squares if tuple(token[1:]) in adjacent_coordinates]
...
def find_nxn_surrounding_squares(coordinate, n):
    ...
    return [(i, j) for i in range(x - n, x + n + 1) for j in range(y - n, y + n + 1) if i >= 0 and j >= 0]
