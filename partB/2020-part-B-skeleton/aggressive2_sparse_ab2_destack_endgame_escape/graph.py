...
from aggressive2_sparse_ab2_destack_endgame_escape.util import *
import copy


def generate_adjacency_list(layout, adjacency_func, our_colour):
    """Return a dictionary of adjacency list
    :param squares: list of square coordinates
    :type squares: list
    """

    our_squares = layout[our_colour]
    adjacency_list = {}
    for current_square in our_squares:
        adjacency_list[tuple(current_square)] = []
        for other_square in our_squares:
            if other_square != current_square and other_square in adjacency_func(1, current_square,
                                                                                 layout, our_colour):
                adjacency_list[tuple(current_square)].append(other_square)
...
def minimax(layout, exp, depth, alpha, beta, maximizing_player, colour, our_colour, escape):
    ...
    if sum(t[0] for t in layout[opponent_colour]) == 0:
        return 100, None, escape

    if depth == 0 or sum(t[0] for t in layout[our_colour]) == 0:
        if our_colour == "whites":
            # return sum(t[0] for t in layout["whites"]) - sum(t[0] for t in layout["blacks"]), None, escape
            return sum(t[0] for t in exp["blacks"]) - sum(t[0] for t in exp["whites"]), None, escape

        elif our_colour == "blacks":
            # return sum(t[0] for t in layout["blacks"]) - sum(t[0] for t in layout["whites"]), None, escape
            return sum(t[0] for t in exp["whites"]) - sum(t[0] for t in exp["blacks"]), None, escape

    if maximizing_player:
        max_eval = -13
        max_action = None
        action_layout_list = generate_all_layouts(layout, exp, colour)
        for t, a, l, exp in action_layout_list:
            eval, _, escape = minimax(l, exp, depth - 1, alpha, beta, False, colour, our_colour, escape)
            if depth == 3 and a[0] == "MOVE" and eval < 0:
                escape = True
            ...
            if beta <= alpha:
                break
        return max_eval, max_action, escape
    else:
        min_eval = 13
        min_action = None
        action_layout_list = generate_all_layouts(layout, colour)
        for t, a, l, exp in action_layout_list:
            eval, _, escape = minimax(l, exp,  depth - 1, alpha, beta, True, colour, our_colour, escape)
            print(a, exp, eval)
            ...
            if beta <= alpha:
                break
        return min_eval, min_action, escape


def generate_all_layouts(layout, exp, colour):
    action_layout_list = []
    ...
    for token in layout[colour]:
        ...
        for destination in destinations:
            for numToken in range(token[0]):
                ...
                next_layout, exploded_token_dict = update_layout(action, layout_copy, colour)
                action_layout_list.append((token, action, next_layout, exploded_token_dict))
...
def update_layout(action, layout, colour):
    ...
    if action[0] == "MOVE":
        ...
        exploded_token_dict = {"whites":[],"blacks":[]}
    ...
    return layout, exploded_token_dict
