...
from aggressive3_sparse_ab_destack_endgame_escape.graph import *
...
class ExamplePlayer:
    ...
    def action(self):
        ...
        def find_nearby_opponents(opponent_colour):
            nearby = {"whites": [], "blacks": []}
            for opp in self.layout[opponent_colour]:
                surroundings = find_nxn_surrounding_squares(opp[1:], opp[0] + 1)
                for surrounding in surroundings:
                    for our in self.layout[our_colour]:
                        if our[1] == surrounding[0] and our[2] == surrounding[1]:
                            nearby[our_colour].append(our)
                            nearby[opponent_colour].append(opp)
            return nearby
        ...
        nearby = find_nearby_opponents(opponent_colour)
        action = None
