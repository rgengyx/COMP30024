import random
from random_sparse_advantage_ab.square import *
from random_sparse_advantage_ab.graph import *
    ...
    def action(self):
        ...
        def find_nearby_opponents(our_colour):
            ...
            for our in self.layout[our_colour]:
                ...
                # surroundings = find_3x3_surrounding_squares(our[1:])
        ...
        if not nearby_opponents:
            ...
            # Check if we have components that are at disadvantage
            isAtDisAdvantage = False
            for token in self.layout[our_colour]:
                coord = token[1:]
                exploded_token_dict = get_exploded_dict(coord, self.layout)

                if len(exploded_token_dict[opponent_colour]) > 0 and sum(
                        t[0] for t in exploded_token_dict[opponent_colour]) < sum(
                        t[0] for t in exploded_token_dict[our_colour]):
                    isAtDisAdvantage = True
                    break
            if isAtDisAdvantage:
                # Use Minimax to deal with disadvantage
