...
from random_sparse_toopp_oppnear_ab_destack.graph import *
    ...
    def action(self):
        ...
        def find_nearby_opponents(opponent_colour):
            ...
            for opp in self.layout[opponent_colour]:
                surroundings = find_nxn_surrounding_squares_boomable(opp[1:], opp[0] + 1)
                for surrounding in surroundings:
                    for our in self.layout[our_colour]:
                        if our[1] == surrounding[0] and our[2] == surrounding[1]:
                            nearby_opponents.append(our)
            return nearby_opponents

        def find_nearest_opponent(token, destinations):
            min_man_dist = 17
            min_d = None
            for d in destinations:
                dist = abs(token[1] - d[1]) + abs(token[2] - d[2])
                if dist < min_man_dist:
                    min_man_dist = dist
                    min_d = d
            return min_d
        ...
        def find_sparse_destination():
            ...
            # Filter destinations that are aways from opponents
            nearest_opp = find_nearest_opponent(token, self.layout[opponent_colour])

            curr_man_dist = abs(nearest_opp[1] - token[1]) + abs(nearest_opp[2] - token[2])

            destinations = [d for d in destinations if abs(nearest_opp[1] - d[1]) + abs(nearest_opp[2] - d[2]) < curr_man_dist]
        ...
        nearby_opponents = find_nearby_opponents(opponent_colour)
