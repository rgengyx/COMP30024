import random
from aggressive_stopper_sparse_ab_destack_endgame_escape.square import *
from aggressive_stopper_sparse_ab_destack_endgame_escape.graph import *
    ...
    def action(self):
        ...
        def find_nearby_opponents():
            ...
            for our in self.layout[our_colour]:
                surroundings = find_nxn_surrounding_squares(our[1:], 1)
        ...
        def find_best_action(layout):
            ...
            max_sparse = -1
            min_dist = 17
            for t, a, l in action_layout_list:
                ...
                if a[0] == "MOVE":
                    ...
                    if new_dist == 1:
                        ...
                        next_layout = update_layout(a, layout_copy, our_colour)
                        coord = (a[3][0], a[3][1])
                        exploded_token_dict = get_exploded_dict(coord, next_layout)
                        if sum(t[0] for t in exploded_token_dict[our_colour]) > sum(t[0] for t in exploded_token_dict[opponent_colour]):
                            eval, action, escape = minimax(self.layout, 3, -13, 13, True, opponent_colour, our_colour,
                                                           False)
                            return action
        ...
        if not nearby[opponent_colour]:
            ...
            # Opponent are approaching
            if self.opponent_action != None and self.opponent_action[0] == "MOVE":
                opp_action = self.opponent_action
                closestOur = None
                closestOurDist = 17
                for w in self.layout[our_colour]:
                    dist = abs(opp_action[2][0] - w[1]) + abs(opp_action[2][1] - w[2])
                    if dist < closestOurDist:
                        closestOurDist = dist
                        closestOur = w

                if opp_action[0] == "MOVE":
                    new_dist = abs(opp_action[3][0] - closestOur[1]) + abs(opp_action[3][1] - closestOur[2])
                    old_dist = abs(opp_action[2][0] - closestOur[1]) + abs(opp_action[2][1] - closestOur[2])

                    if new_dist < old_dist:

                        action_layout_list = generate_all_layouts(self.layout, our_colour)
                        ...
                        for opp in self.layout[opponent_colour]:
                            if opp[1] == opp_action[3][0] and opp[2] == opp_action[3][1]:
                                closestOpponent = opp
                                break

                        for t, a, l in action_layout_list:

                            if t == closestOur and a[0] == "MOVE":
                                new_dist = abs(a[3][0] - closestOpponent[1]) + abs(a[3][1] - closestOpponent[2])
                                old_dist = abs(a[2][0] - closestOpponent[1]) + abs(a[2][1] - closestOpponent[2])
                                ...
                                if new_dist == 1:
                                    ...
                                    coord = (a[3][0], a[3][1])
                                    exploded_token_dict = get_exploded_dict(coord, next_layout)
                                    if sum(t[0] for t in exploded_token_dict[our_colour]) > sum(
                                            t[0] for t in exploded_token_dict[opponent_colour]):
                                        eval, action, escape = minimax(self.layout, 3, -13, 13, True, opponent_colour,
                                                                       our_colour,
                                                                       False)
                                        return action
            ...
            # Minimax
            eval, action, escape = minimax(self.layout, 3, -13, 13, True, opponent_colour, our_colour, False)
            if eval > self.old_eval or action[0] == "BOOM":
                self.old_eval = eval
                return action

            diff = sum(t[0] for t in self.layout[our_colour]) - sum(t[0] for t in self.layout[opponent_colour])
            if eval == diff and escape == False:
                ...
                if not nearby[our_colour]:
                    action = find_best_action(self.layout)
                else:
                    for our in nearby[our_colour]:
                        action = ("BOOM", (our[1], our[2]))
                        break
            elif eval > diff and escape == False:
                action = find_best_action(self.layout)
